...
public class SketchBottom extends EmbeddedSketch {
	...
	private void drawNoGroup() {

		String noCluster = "No group selected...";
		noStroke();
		fill(220);
		rectMode(PApplet.CORNER);
		rect(20, 15, myWidth - 40, myHeight - 30, 50, 50);
		fill(255);
	...
	private void drawQuestionTitle(QuestionData qData, float x1, float y1,
		...
		textAlign(PApplet.LEFT, PApplet.CENTER); // Sufficiently far from the
													// text() call so that
													// processing has time to
	...
	private void drawToolTipQuestion(QuestionData qData) {
		...
		textSize(12);
		textAlign(PApplet.CENTER, PApplet.CENTER);// Sufficiently far from the
													// text() call so that
													// processing has time to
													// set it before drawing
		...
		rect(tooltipX, tooltipY, textWidth(tooltip) * (float) 1.1, 20, 5, 5);
