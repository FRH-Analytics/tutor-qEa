...
public class ControlFont extends PApplet {
	...
	public void setup() {
		...
		cp5.addButton("button").setValue(10).setPosition(20, 20)
				.setSize(100, 30).setId(1);

		b = cp5.addButton("buttonValue").setValue(4).setPosition(100, 190)
				.setSize(200, 200).setId(2);
		...
		// to display a font as smooth or non-smooth, use true/false as 3rd
		// parameter
		// when creating a PFont:

		PFont pfont = createFont("Arial", 20, true); // use true/false for
														// smooth/no-smooth
		controlP5.ControlFont font = new controlP5.ControlFont(pfont, 241);

		// (3)
		// change the font and content of the captionlabels
		// for both buttons create earlier.
		cp5.getController("button").getCaptionLabel().setFont(font)
				.toUpperCase(false).setSize(24);

		b.captionLabel().setFont(font).setSize(50).toUpperCase(false)
				.setText("hello");
		//
		// (4)
	...
	public void button(float theValue) {
		...
		isOpen = !isOpen;
		cp5.controller("button").setCaptionLabel(
